...
async def search(
    ...
    try:
        ...
        for record in raw_results:
            ...
            metadata = {}
            record_metadata = record.get("metadata") if isinstance(record, dict) else getattr(record, "metadata", None)
            if record_metadata:
                try:
                    metadata = json.loads(record_metadata) if isinstance(record_metadata, str) else record_metadata
                except (json.JSONDecodeError, TypeError):
                    metadata = {}

            # Get similarity score (_distance is returned by LanceDB)
            score = record.get("_distance", 0.0) if isinstance(record, dict) else getattr(record, "_distance", 0.0)

            # Safe extraction from records with defaults when None
            results.append(SearchResult(
                id=record.get("id", "") if isinstance(record, dict) else getattr(record, "id", ""),
                text=record.get("text", "") if isinstance(record, dict) else getattr(record, "text", ""),
                file_id=record.get("file_id", "") if isinstance(record, dict) else getattr(record, "file_id", ""),
                chunk_index=record.get("chunk_index", 0) if isinstance(record, dict) else getattr(record, "chunk_index", 0),
                metadata=metadata,
                score=score
            ))
